...
GEOGRAPHY IS A HARD CONSTRAINT FOR TARGET ACCOUNTS AND LOOKALIKE PROSPECTS:
- Canada: targets must be headquartered in Canada OR have substantial Canadian operations with a plausible Canadian buying organization. Do not return U.S.-only, Mexico-only, LATAM-only, European-only, or APAC-only companies as targets.
- LATAM: targets must be headquartered in, or have substantial relevant operations within, Mexico, Central America, South America, or the Caribbean. Do not return Canada-only or U.S.-only companies as targets.
- North America: targets may be in the United States, Canada, or Mexico.
- Global: no geographic restriction.
Competitors and partners may be outside the selected geography when relevant, but label that clearly.
Do not broaden or reinterpret the selected geography.
...
SCHEMA = {
 ...
 "target_accounts":[{"company":"","why_fit":"","observable_signal":"","priority":"PURSUE","confidence":"medium","geography_fit":""}],
 "lookalike_prospects":[{"company":"","similar_to":"","shared_attributes":[],"why_now":"","priority":"TEST","geography_fit":""}],
...
def geo_instruction(geography):
    rules = {
        "Canada": "TARGET TERRITORY = CANADA ONLY. Target accounts and lookalikes must be headquartered in Canada or have substantial relevant Canadian operations with a plausible Canadian buying organization. Do not include Mexico or U.S.-only companies.",
        "LATAM": "TARGET TERRITORY = LATIN AMERICA. Include Mexico, Central America, South America, and the Caribbean. Target accounts and lookalikes must be headquartered in or have substantial relevant operations in LATAM. Do not include Canada-only or U.S.-only companies.",
        "North America": "TARGET TERRITORY = NORTH AMERICA. Target accounts and lookalikes may be in the United States, Canada, or Mexico.",
        "Global": "TARGET TERRITORY = GLOBAL. No geographic restriction."
    }
    return rules[geography]
...
MANDATORY GEOGRAPHIC FILTER: {geo_instruction(vals['geography'])}
...
For every target account and lookalike, explain why it qualifies geographically in the geography_fit field. Before returning JSON, self-check target_accounts and lookalike_prospects and remove any entry that violates the selected geography.
...
with st.form("venture_form"):
    ...
    with c1:
        ...
        geography=st.selectbox("Primary geography", ["Canada","LATAM","North America","Global"])
    with c2:
